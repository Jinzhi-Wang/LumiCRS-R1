...
def parse_args():
    ...
    parser.add_argument("--output_dir", type=str, default='/media/wjz/2TBverydiao/ECR/src_emo/data/saved/rec', help="Where to store the final model.")
    ...
    parser.add_argument("--tokenizer", type=str,default="/media/wjz/2TBverydiao/ECR/src_emo/save/dialogpt/")
    parser.add_argument("--text_tokenizer", type=str,default="/media/wjz/2TBverydiao/ECR/src_emo/save/roberta/")
    # model
    parser.add_argument("--model", type=str, required=False,default="/media/wjz/2TBverydiao/ECR/src_emo/save/dialogpt/",
                        help="Path to pretrained model or model identifier from huggingface.co/models.")
    parser.add_argument("--text_encoder", type=str,default="/media/wjz/2TBverydiao/ECR/src_emo/save/roberta/")
    parser.add_argument("--num_bases", type=int, default=8, help="num_bases in RGCN.")
    parser.add_argument("--n_prefix_rec", type=int)
    parser.add_argument("--prompt_encoder", type=str) # or "data/saved/rec/" for test stage in recommendation
    # optim
    ...
    if args.test:
        ...
        for batch in tqdm(valid_dataloader):
            with torch.no_grad():
                ...
                all_ranks = torch.argsort(logits, dim=-1, descending=True).tolist()
                ranks = [[kg['item_ids'][rank] for rank in batch_rank] for batch_rank in ranks]
                all_ranks = [[kg['item_ids'][rank] for rank in batch_rank] for batch_rank in all_ranks]
                evaluator.evaluate(all_ranks, labels, True)
