...
def focal_loss(logits, targets, alpha=0.25, gamma=2.0, reduction='mean'):
    ...
    参数:
    - logits: 预测的 logits (未经过 softmax) [batch_size, num_classes]
    - targets: 真实的类别索引 (不需要 one-hot 编码) [batch_size]
    - alpha: 平衡因子，默认为 0.25，用于调整正负样本的重要性
    - gamma: 调节因子，默认为 2.0，控制难易样本的损失贡献
    - reduction: 是否对损失进行均值 ('mean') 或求和 ('sum')

    返回:
    - focal_loss: 计算后的 Focal Loss
    """
    # 计算 softmax 概率
    probs = F.softmax(logits, dim=-1)  # [batch_size, num_classes]

    # 获取真实类别的概率 (即 p_t)
    targets_one_hot = F.one_hot(targets, num_classes=logits.shape[-1])  # [batch_size, num_classes]
    pt = (probs * targets_one_hot).sum(dim=-1)  # 取出目标类别的概率值 [batch_size]

    # 计算 Focal Loss: -α * (1 - pt)^γ * log(pt)
    focal_weight = (1 - pt) ** gamma  # 计算焦点因子
    focal_loss = -alpha * focal_weight * torch.log(pt + 1e-10)  # 加上一个小的数防止 log(0)
...
class PromptGPT2forCRS(GPT2PreTrainedModel):
    ...
    def forward(
        ...
        if rec:
            ...
            # 修改原代码，使其使用 Focal Loss
            if rec_labels is not None and loss_weight is not None and weighted_loss == True:
                rec_loss = focal_loss(rec_logits, rec_labels, alpha=0.25, gamma=2.0, reduction='none')  # 计算 Focal Loss
                rec_loss = torch.einsum('l,l->l', rec_loss, loss_weight)  # 进行损失加权
                rec_loss = torch.mean(rec_loss)  # 计算加权后的均值损失
            else:
                rec_loss = focal_loss(rec_logits, rec_labels, alpha=0.25, gamma=2.0)  # 计算 Focal Loss 并进行默认均值化
